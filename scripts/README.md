...
| `seed_source_aliases.py`        | **Yes**       | No           | UPSERTs `source_name_aliases` from a reviewed suggestions CSV. Idempotent. Curated input lives in `data/source_name_aliases.csv`. **The table was empty in prod until 2026-08-17**, which was invisible because both `getRecentArticlesByOutletSlug` and `getTermCoverage` fall back to matching `outlet_profiles.name` directly — so the 43 outlets whose feed name equals their dossier name resolved anyway and nothing reported the other 41.8% of articles. The committed CSV holds only the 14 rows that *change* behaviour (name variants like `The Guardian US` → `the-guardian`); the exact-name matches the audit also proposes are omitted as no-ops. Effect measured on `/term/temporary-protected-status`: the left bucket went 21 → 51, centre 39 → 45, right unchanged at 46, unplaced 40 → 4. **The misses were not random** — the outlets filing under variant names skew left, so the gap made the corpus look right-leaning; that asymmetry is why this is a D37 matter and not housekeeping. Review new rows by hand: the audit's `substring` tier is a heuristic, not data, and proposed `IGN → foreign-affairs` (matching "ign" inside "foreign"). |
...
| `seed_term_profiles.py`         | **Yes**       | No           | Migration 031. UPSERTs `term_profiles` from `data/term_profiles.csv` — the curated definitions behind `/term/<slug>`. **Hand-written on purpose.** `articles.context_primer` already defines ~11,900 terms, but every `terms[].source` in the corpus is null, and a standalone page stating what a legal term means without a citation is the defect migrations 013 and 015 each had to remove. So the seeder drops any row missing a `definition_source`, or carrying one that isn't https. Also drops a malformed `aliases` cell or an alias a second term already claims — aliases are match keys in the read-time coverage query, so a bad one silently changes what the page claims to cover. Bad rows are dropped, never fatal to the run. Unlike the politician seeder, `--prune` here really does DELETE: nothing references a term row and the CSV is its source of truth. Idempotent; `--dry-run` supported. | **24 terms as of 2026-08-17; 20 clear the publish floor.** Two known limits of the read-time coverage query, both measured rather than assumed: (1) **it searches title + summary only, so it misses terms that live in article bodies** — `prior restraint` matches 0 headlines but is defined in 128 primers, `certiorari` 5 vs 83, `qualified immunity` 6 vs 75, `cloture` 0 vs 45. The terms most worth explaining are precisely the ones a journalist uses in paragraph nine, so the floor currently withholds them for lack of a signal rather than lack of coverage. **Fixed in the same PR by migration 033**, which indexes the primer's term list so the coverage query can union it in: all 24 terms now clear the floor, and `prior-restraint` went from 0 to 128. The primer is a coverage signal only — every primer term in the corpus carries `source: null`, which is why `term_profiles` is hand-written. (2) **No US scope filter.** Definitions are US law; the query is not, so a foreign pardon or a foreign judge's recusal is counted — observed in both. Fine for `redistricting` or `cloture`, which are US-specific in practice; noted per-row where it was seen. **Ends by naming any term with no measured coverage**, because a freshly seeded term has `article_count` NULL until `refresh_term_coverage.py` runs and the floor reads NULL as zero — fail-closed working as designed, and indistinguishable from a bug unless something says so.
| `refresh_term_coverage.py`     | **Yes**       | No           | Migration 034. Recomputes `term_profiles`' `article_count` / `outlet_count` / `unnamed_count` and stamps `coverage_computed_at`. **Run after every term seed, and on the seeders' cadence.** `/glossary` and the publish floor read what this writes; `/term/<slug>` still computes live and is unaffected. Exists because the read-time query is a lateral over term × surface-form × corpus and got worse as the table grew — 785 ms at 24 terms, **1,522 ms at 37**, versus **66 ms** reading the columns. A sourced term with no matching article is written as an explicit `0` with a fresh stamp, never left NULL: "measured, found nothing" and "never measured" must stay distinguishable, because the floor treats NULL as zero and would otherwise withhold the term forever. Reports which counts moved and, separately, any **floor crossings** — those change what is indexed. This script's SQL is the single definition of coverage and must agree with `termMatchSql` in `sift/lib/db.ts`. Idempotent; `--dry-run`. | **`--check` reports staleness without recomputing** — never-measured terms (named, because those are silently unpublished) and counts older than 30 days — and exits 2 if a refresh is needed, so a human or a cron can branch on it. `seed_term_profiles.py` prints the same warning after seeding, since seeding a term does not publish it.
