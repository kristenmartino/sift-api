...
| `output_stop_summary.py`        | No (read-only) | No          | Reads `llm_output_stops` (migration 021) and answers one question: are `summarizer.batch`'s alignment re-asks caused by output truncation? Those re-asks run at **4-12% of calls** (measured 2026-08-11 as the excess of calls over `ceil(articles / BATCH_SIZE)`) and each is a full-price duplicate, but nothing recorded why. `MAX_OUTPUT_TOKENS = 700` is comfortable against five ~60-token summaries and tight against five 79-word ones (~575 + JSON scaffolding), and a response cut off at the cap is truncated JSON — which fails index alignment exactly like a scrambled one, except the fix is a bigger ceiling rather than a re-ask. Splits on `aligned`, because the question is not whether calls ever hit the cap but whether the **misaligned** ones did. Exit **0** truncation is a cause (raise the ceiling), **2** it is not (leave it alone), **3** too few calls to read a rate (`MIN_CALLS = 100`) — a verdict on the first handful of rows is the failure `shadow_summary.py`'s `MIN_RUNS` guards against. `--days` supported. |
