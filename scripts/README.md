...
| `seed_politician_profiles.py`   | **Yes**       | No           | UPSERTs `politician_profiles` from `data/politician_profiles.csv`. Idempotent. **Does not own `interest_group_ratings`** — that column belongs to `seed_lcv_scores.py`, and the roster CSV still carries the pre-019 `{}` for every row, so a plain assignment would wipe it (532 LCV entries were one run away from this on 2026-08-07). The upsert keeps a non-empty stored array and otherwise takes the incoming value, which is coerced to `[]`; guarding on the *incoming* value instead would make a bad stored value immortal. Does **not** prune, so a member who left Congress stays until their `chamber` is set to `former` by hand. |
