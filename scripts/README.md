...
| `refresh_politician_donors.py`  | No (updates the CSV) | No     | Phase 3.F.2: refreshes `top_industries_current_cycle` for each politician via the OpenSecrets `candIndustry` API. **Requires `OPENSECRETS_API_KEY`** — silently no-ops without it. Free tier is 200 calls/day; supports `--max N` + `--offset M` to rotate across days. `--dry-run` flag for spot-checks. Re-run-safe (preserves committees/notes/etc.). |
